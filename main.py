...
def build_dns_response(packet_id, opcode, rd, qdcount, questions, answers):
    ...
    nscount = (
        0  # 16 bits: Number of records in the Authority section. Expected value: 0.
    )
    arcount = (
        0  # 16 bits: Number of records in the Additional section. Expected value: 0.
    )
    ...
    while True:
        try:
            ...
            for question in questions:
                ...
                single_query = struct.pack(
                    "!HHHHHH", packet_id, opcode << 11 | rd << 8, 1, 0, 0, 0
                )
                single_query += encode_domain_name(domain_name) + struct.pack(
                    "!HH", qtype, qclass
                )
            ...
            response = build_dns_response(
                packet_id, opcode, rd, qdcount, questions, answers
            )
